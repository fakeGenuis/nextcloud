#+TITLE: Nextcloud in docker with self-signed and public https

Runs Nextcloud in docker behind Caddy: a self-signed certificate serves a
private hostname for direct/LAN access, and an optional public hostname with a
trusted certificate serves external access (for example through an frp tunnel).

* Requirements
1. ~docker~ and ~docker-compose~
2. ~openssl~
3. optional: ~ssl-cert-check~ (falls back to =openssl -checkend= when absent)
...
render the templates with =envsubst=, keeping a timestamped backup of the
previous output before overwriting it.
#+begin_src bash
